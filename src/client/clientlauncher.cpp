...
bool ClientLauncher::run(GameParams &game_params, const Settings &cmd_args)
{
	...
	porting::setXorgClassHint(video_driver->getExposedVideoData(), "Blokel");
	...
	*/
	...
	while (device->run() && !*kill && !g_gamecallback->shutdown_requested)
	{
		...
		const wchar_t *text = wgettext("Launcher");
		device->setWindowCaption((std::wstring(L"Blokel [") + text + L"]").c_str());
		delete[] text;
