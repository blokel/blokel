...
public class BlokelAssetCopy extends Activity
{
	@Override
	public void onCreate(Bundle savedInstanceState)
	{
		...
		BlokelAssetCopy prevActivity = 
				(BlokelAssetCopy) getLastNonConfigurationInstance();
	...
	private class copyAssetTask extends AsyncTask<String, Integer, String>
	{
		...
		protected String doInBackground(String... files)
		{
			...
			// prepare temp folder
			File TempFolder = new File(baseDir + "Blokel/tmp/");
			...
			if (!TempFolder.exists())
			{
				...
				{
					Log.v("BlokelAssetCopy","deleting: " + todel[i].getAbsolutePath());
					todel[i].delete();
				}
			}
			...
			// add a .nomedia file
			try {
				OutputStream dst = new FileOutputStream(baseDir + "Blokel/.nomedia");
				dst.close();
			} catch (IOException e) {
				Log.e("BlokelAssetCopy","Failed to create .nomedia file");
				e.printStackTrace();
			}
			...
			for (int i = 0; i < m_tocopy.size(); i++)
			{
				try
				{
					...
					try
					{
						...
						Log.e("BlokelAssetCopy","Copying file: " + filename + " FAILED (not in assets)");
						e.printStackTrace();
						continue;
					}
					...
					if (len > 0)
					{
						...
						try
						{
							...
							Log.e("BlokelAssetCopy","Copying file: " + baseDir +
						...
						dst.close();
						Log.v("BlokelAssetCopy","Copied file: " +
									m_tocopy.get(i) + " (" + total_filesize +
									" bytes)");
					}
					else if (len < 0)
					{
						Log.e("BlokelAssetCopy","Copying file: " +
			...
			while (itr.hasNext())
			{
					...
					if (!current_folder.exists())
					 {
						...
						{
							Log.e("BlokelAssetCopy","\t failed create folder: " +
									FlashPath);
						}
						else
						{
							Log.v("BlokelAssetCopy","\t created folder: " +
									FlashPath);
						}
					}
				...
				if (testme.exists())
				{
					...
					catch (IOException e)
					{
						...
						Log.e("BlokelAssetCopy","Failed to open asset file \"" +
		...
		protected void BuildFolderList()
		{
			...
			{
				Log.e("BlokelAssetCopy","Error on processing index.txt");
				e1.printStackTrace();
			}
		}
		...
		protected void BuildFileList()
		{
			...
			{
				Log.e("BlokelAssetCopy","Error on processing filelist.txt");
				e1.printStackTrace();
			}
		}
